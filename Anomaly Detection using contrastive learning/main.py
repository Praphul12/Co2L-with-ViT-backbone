...
root = r"C:\AlgoZenith\AI\Co2L\TinyImageNet"
def parse_option():
    parser = argparse.ArgumentParser('argument for training')

    # Add the argument definitions as per your original code
    ...
    parser.add_argument('--dataset', type=str, default='cifar10', choices=['cifar10', 'tiny-imagenet', 'path'])
    ...
    # Additional processing for paths and other settings
    ...
    opt.model_name = '{}_{}_{}_lr_{}_decay_{}_bsz_{}_temp_{}_trial_{}_{}_{}_{}_{}_{}'.format(
        opt.dataset, opt.size, opt.model, opt.learning_rate,
        opt.weight_decay, opt.batch_size, opt.temp, opt.trial,
        opt.start_epoch if opt.start_epoch is not None else opt.epochs, opt.epochs,
        opt.current_temp, opt.past_temp, opt.distill_power
    )
...
def set_replay_samples(opt, model, prev_indices=None):
    ...
    # Create a custom Dataset class to handle indices
    class IdxDataset(Dataset):
        def __init__(self, dataset, indices):
            self.dataset = dataset
            self.indices = indices
        def __len__(self):
            return len(self.dataset)  # Return length of indices, not dataset
    ...
    print(f"Previous indices before processing: {prev_indices}")
    print(f"Total targets: {len(val_targets)}")
    # If prev_indices is None, initialize it as an empty list for the first task
    if prev_indices is None:
        ...
        if len(prev_indices) > 0:
            ...
            print(f"After processing prev_indices: {prev_indices}")
    ...
    if len(observed_classes) == 0:
        return prev_indices  # Return prev_indices if there are no observed classes
    ...
    print(f"Selected observed indices: {len(selected_observed_indices)}")
...
def set_model(opt):
    ...
    # Remove synchronized Batch Normalization (syncBN) related code
    # No need for apex.parallel.convert_syncbn_model
...
def train(train_loader, model, model2, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, labels) in enumerate(train_loader):
        ...
        # images = images.to('cpu', non_blocking=True)  # Moves images to CPU
        # labels = labels.to('cpu', non_blocking=True)  # Moves labels to CPU
